import {
  ...
  calendarWeekSummary, evolutionForPlan, monthAggregate, personalRecordsInPeriod, HistoryBundle,
} from "../src/lib/calc";
...
console.log("\n10) PRs REAIS DO MÊS (primeira carga é baseline)");
const prsMes = mk([
  {date:"2026-06-28",wk:1,key:"A",done:true,ex:[["supino","Peito",50,10]]},
  {date:"2026-07-05",wk:2,key:"A",done:true,ex:[["supino","Peito",55,10]]},
  {date:"2026-07-12",wk:3,key:"A",done:true,ex:[["supino","Peito",60,8]]},
]);
const eventosMes = personalRecordsInPeriod(prsMes, (date) => date.startsWith("2026-07"));
const resumoMes = monthAggregate(prsMes, "2026-07-12");
t("conta os dois recordes do mês", eventosMes.length === 2 && resumoMes.prs === 2);
t("lista o movimento uma única vez", resumoMes.prNomes.length === 1 && resumoMes.prNomes[0] === "supino");
